...
.bx-pager, .bx-controls-auto  { display:none;}
      ...
      <style>
		...
		<div class="boxed">
			...
			<h1 class="title2"><?php echo isset($data->test_title) ? $data->test_title : ''; ?></h1>
        ...
        <table class="boxed orange" width="92%" style="background: rgba(171, 32, 0, 0.58);padding:5px;">
          <tr>
            <td width="60%" class="title3" > Last Date to apply</td>
          ...
          <tr>
            <td width="60%" class="title3"> Download Test Advertisement</td>
            <td width="40%" class="title3" style="border-left:1px solid white; padding-left:10px;"> 
            	<?php if(isset($data->test_ad) && $data->test_ad != '' && is_file(Yii::getPathOfAlias('webroot') ."/uploads/test_ads/".$data->test_ad)){ ?>
                    <a  class="btn btn-primary" href="<?php echo Yii::app()->request->baseurl ."/uploads/test_ads/".$data->test_ad; ?>" download>Download Test Ad</a>
                <?php }else { ?>

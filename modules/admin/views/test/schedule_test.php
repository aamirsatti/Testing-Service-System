                                    ...
                                    ?>    
                                        <tr class="<?php if($counter%2 == 0) echo 'odd gradeX';?>">
                                            ...
                                            <td><?php 
												 ...
												 if(!empty($rec->posts)) 
												 {
													 foreach($rec->posts as $post) 
											      		$app = $app + Pstcandloctestcentr::model()->count('PID=:id and status=:status', array(':id' => $post->PID, ':status' => 1));
												 }
												  echo isset($app) ? $app : 0; 
												 ?>
                                            </td>
                                            <td><?php echo isset($rec->TestDate) ? date('d M, Y', strtotime($rec->TestDate)) : ''; ?>
                                            <td class="center" >
                                                <a href="<?php echo Yii::app()->createurl('admin/test/schedule_test_detail/id/'.base64_encode($rec->TID));?>" title="View Detail"> <i class="fa fa-search"></i></a> 
                                                <!-- <a href="javascript:void(0);" onclick="product_del('<?php //echo Yii::app()->createurl('admin/test/delete/id/'.base64_encode($rec->TID));?>');" title="Delete" data-toggle="modal" data-target="#myModal"> <i class="fa fa-times"></i></a> -->
                                            </td> 
                                        </tr>
                                    <?php  $counter++;
                                    }
